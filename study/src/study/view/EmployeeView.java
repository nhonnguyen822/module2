...
public class EmployeeView {
    ...
    public static Employee inputDataAddEmployee() {
        System.out.println("nhập mã nhân viên");
    ...
    //nhập thay đổi của nhân viên

    public static Employee inputDataEditEmployee(Employee employee) {
        String emlloyeeId = employee.getId();
        System.out.println("tên nhân viên hiện tại" + employee.getName());
        System.out.println("nhập tên nhân viên");
        String name = scanner.nextLine();
        while (true) {
            String regex = "^([A-Z][a-z]*)(\\s[A-Z][a-z]*)?(\\s[A-Z][a-z]*)+$";
        ...
        System.out.println("ngày sinh hiện tại " + employee.getBirtDay());
        DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        LocalDate birthDay = null;
        while (true) {
            System.out.println("nhập ngày sinh với đinh dạng dd/MM/yy");
            ...
            try {
                ...
                LocalDate toDay = LocalDate.now();
                Period age = Period.between(birthDay, toDay);
                if (age.getYears() > 18 && age.getYears() < 60) {
                    break;
                } else {
                    System.out.println("tuổi của nhân viên không được nhỏ hơn 18 và lớn hơn 60");
                }
            } catch (DateTimeException e) {
                System.out.println("ngày sinh không hợp lệ");
            }
        ...
        Gender gender = null;
        while (gender == null) {
            System.out.println("giới tinh hiện tại " + employee.getGender());
            System.out.println("nhập giới tính");
            String input = scanner.nextLine();
            gender = Gender.fromToString(input);
            if (gender == null) {
        ...
        System.out.println("số CMND /CCCD hiện tại " + employee.getNumberCMND());
        System.out.println("nhập số CMND /CCCD");
        String numberCMND = scanner.nextLine();
        while (true) {
            String regexCMND = "^\\d{9}(\\d{3})?$";
        ...
        System.out.println("số điện thoại hiện tại " + employee.getNumberPhone());
        System.out.println("nhập số điện thoại ");
        String numberPhone = scanner.nextLine();
        while (true) {
            String regexSoDT = "^(\\d{2}-)?(0\\d{9})$";
        ...
        System.out.println("nhập email hiện tại " + employee.getEmail());
        System.out.println("nhập email ");
        String email = scanner.nextLine();
        while (true) {
            String regexEmail = "^[\\w.-]+@([a-zA-Z]{3,6})(\\.[a-zA-Z]{2,6}){1,2}$";
        ...
        TrinhDo trinhDo = null;
        while (trinhDo == null) {
            System.out.println("trình độ hiện tại của nhân viên " + employee.getTrinhDo());
            System.out.println("nhập trình độ của nhân viên");
            String input = scanner.nextLine();
            trinhDo = TrinhDo.fromToString(input);
            if (trinhDo == null) {
        ...
        ViTri viTri = null;
        while (viTri == null) {
            System.out.println("vị trí hiện tại của nhân viên " + employee.getViTri());
            System.out.println("nhập vị trí  của nhân viên");
            String input = scanner.nextLine();
            viTri = ViTri.fromToString(input);
            if (viTri == null) {
        ...
        int salary = 0;
        try {
            System.out.println("lương hiện tại của nhân viên " + employee.getSalary());
            System.out.println("nhập lương của nhân viên");
            salary = Integer.parseInt(scanner.nextLine());
            while (salary <= 0) {
                System.out.println("nhập lương của nhân viên phai lon hon 0");
        ...
        return new Employee(emlloyeeId, name, birthDay, gender, numberCMND, numberPhone, email, trinhDo, viTri, salary);
    }
}
