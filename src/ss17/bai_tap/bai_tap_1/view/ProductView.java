package ss17.bai_tap.bai_tap_1.view;

import ss17.bai_tap.bai_tap_1.enity.Product;
import java.util.List;
import java.util.Scanner;

public class ProductView {
    private static final Scanner scanner = new Scanner(System.in);

    public static Product inputAddProduct() {
        System.out.println("nhập mã sản phẩm ");
        String maSP = scanner.nextLine();
        System.out.println("nhập tên sản phẩm ");
        String tenSP = scanner.nextLine();
        System.out.println("nhập giá");
        int priceSP = Integer.parseInt(scanner.nextLine());
        System.out.println(" nhập hãng sản xuât");
        String hangSX = scanner.nextLine();
        System.out.println("mô tả sản phầm");
        String moTaSP = scanner.nextLine();
        return new Product(maSP, tenSP, priceSP, hangSX, moTaSP);
    }

    public static void display(List<Product> productList) {
        for (int i = 0; i < productList.size(); i++) {
            System.out.println(productList.get(i));
        }
    }

    public static String inputMaSP() {
        System.out.println("nhập mã sản phẩm");
        return scanner.nextLine();
    }

    public static void search(List<Product> productList, String maSP) {
        for (int i = 0; i < productList.size(); i++) {
            if (productList.get(i).getMaSP().equals(maSP)) {
                System.out.println(productList.get(i));
            }
        }
    }
}
